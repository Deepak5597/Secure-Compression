...
public class DHuffman {
	...
	public void decode(String inputFile, String outputFile) {
        int cr=(100 - (inputFile).length() * 100 / (outputFile).length());
	...
	public double encode(String inputFile, String outputFile) {
		double cr=0;
//		double cr=((100 - (new File(outputFile)).length() * 100 / (new File(inputFile)).length()));
		BufferedInputStream in = null;
		BufferedOutputStream out = null;
		try {
			in = new BufferedInputStream(new FileInputStream(inputFile));
		...
		try {
			...
			in.close();
			out.close();
			cr=((100 - (new File(outputFile)).length() * 100 / (new File(inputFile)).length()));
		} catch (IOException e) {e.printStackTrace(); System.exit(2);}
	return cr;
	...
	public  void insert(int val) {
	 ...
	 Tree root;
	 Tree NYT; // Not Yet Transferred
	 Tree table[]; // fast look up for leaves
	 Tree list[]; // fast look up for all nodes
	 int listTop; // top of the list
